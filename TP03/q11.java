...
class Lista {
    ...
    Celula particionar(Celula low, Celula high) {
        ...
        for (Celula j = low; j != high; j = j.prox) {
            if ((j.perso.getHouse().compareTo(pivo.getHouse()) < 0) || ((j.perso.getHouse().compareTo(pivo.getHouse()) == 0) && (j.perso.getName().compareTo(pivo.getName()) < 0))) {
                i = (i == null) ? low : i.prox;
                trocar(i, j);
            }
        }
...
class Personagem{
    ...
    int compareTo(Personagem other) {
        ...
        return this.house.compareTo(other.house);
    }
    //Método de imprimir
