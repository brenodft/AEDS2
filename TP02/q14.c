...
void radCountingSort(Personagem *array, int n, int exp) {
    int count[256] = {0}; // Há 256 caracteres ASCII possíveis
    Personagem output[n];

    // Contar ocorrências de cada caractere na posição exp
    for (int i = 0; i < n; i++) {
        char c = (strlen(array[i].id) > exp) ? array[i].id[exp] : 0;
        count[(int)c]++;
    }

    // Transformar count em posições acumulativas
    for (int i = 1; i < 256; i++) {
        count[i] += count[i-1];
    }

    // Ordenar de acordo com o caractere na posição exp
    for (int i = n-1; i >= 0; i--) {
        char c = (strlen(array[i].id) > exp) ? array[i].id[exp] : 0;
    ...
    // Copiar os dados ordenados de volta para o array original
    for (int i = 0; i < n; i++) {
        array[i] = output[i];
    }
}
void radixSortPersonagens(Personagem *array, int n) {
    // Encontrar o tamanho máximo do nome entre os personagens
    int maxLen = 0;
    for (int i = 0; i < n; i++) {
        int len = strlen(array[i].id);
        if (len > maxLen) maxLen = len;
    }

    // Chamar radCountingSort para cada posição de caractere dos nomes, da direita para a esquerda
...
int main() {
    ...
    radixSortPersonagens(personagens,numPersonagens);
