...
public class q09 {
    ...
    // método para criar e ajustar um heap máximo
    private static void heapify(Personagem[] arr, int n, int i) {
        int largest = i;
        int left = 2 * i + 1;
        int right = 2 * i + 2;

        // verifica se o filho esquerdo é maior que o pai
        if (left < n && (arr[left].getHairColor().compareTo(arr[largest].getHairColor()) > 0 ||
            (arr[left].getHairColor().compareTo(arr[largest].getHairColor()) == 0 && 
            arr[left].getName().compareTo(arr[largest].getName()) > 0))) {
            largest = left;
        }

        // verifica se o filho direito é maior que o maior elemento até agora
        if (right < n && (arr[right].getHairColor().compareTo(arr[largest].getHairColor()) > 0 ||
            (arr[right].getHairColor().compareTo(arr[largest].getHairColor()) == 0 && 
            arr[right].getName().compareTo(arr[largest].getName()) > 0))) {
            largest = right;
        }

        // se o maior elemento não for o pai, troca-os e ajusta o heap
        if (largest != i) {
            swap(arr, i, largest);
    ...
    public static void heapSort(Personagem[] personagens) {
        ...
        for (int i = n - 1; i >= 0; i--) {
            ...
            // chama o método heapify na subárvore reduzida
