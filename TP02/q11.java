...
public class q11 {
    ...
    private static int getMaior(Personagem[] array) {
        int maior = array[0].getYear();
        for (int i = 1; i < array.length; i++) {
            if (array[i].getYear() > maior) {
                maior = array[i].getYear();
            }
        }
        return maior;
    }

    public static void countingsort(Personagem[] array) {
        try{
        FileWriter writer = new FileWriter("808238_countingsort.txt",true);
        float startTime = System.nanoTime();
        int n = array.length;
        int comp = 0;
        int mov = 0;
        // Maior ano para criar o array de contagem
        int max = getMaior(array) + 1;
        ...
        // Contagem dos anos de cada personagem
        for (int i = 0; i < n; i++) {
            count[array[i].getYear()]++;
            mov++;
            comp++;
        }
    
        // Acumula a contagem para obter posições no array final
        for (int i = 1; i < max; i++) {
            count[i] += count[i - 1];
        }
        ...
        // Ordenação
        for (int i = n - 1; i >= 0; i--) {
            ordenado[count[array[i].getYear()] - 1] = array[i];
            count[array[i].getYear()]--;
            mov++;
        }
        ...
        for (int i = 0; i < n; i++) {
            int j = i;
            // Aplica insertion sort para o desempate por nome dentro do mesmo ano
            while (j > 0 && array[j-1].getYear() == array[j].getYear() && array[j].getName().compareTo(array[j-1].getName()) < 0) {
                Personagem temp = array[j];
                array[j] = array[j-1];
                array[j-1] = temp;
                j--;
            }
