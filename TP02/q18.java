...
public class q18 {
    ...
    public static void quicksort(Personagem[] registros, int esq, int dir, int k){
        if (esq < dir) {
            int i = esq, j = dir - 1;
            Personagem pivo = registros[(esq + dir) / 2];
            while (i <= j) {
                while (registros[i].getHouse().compareTo(pivo.getHouse()) < 0 ||
                       (registros[i].getHouse().equals(pivo.getHouse()) && registros[i].getName().compareTo(pivo.getName()) < 0)) {
                    i++;
                }
                while (registros[j].getHouse().compareTo(pivo.getHouse()) > 0 ||
                       (registros[j].getHouse().equals(pivo.getHouse()) && registros[j].getName().compareTo(pivo.getName()) > 0)) {
                    j--;
                }
                if (i <= j) {
                    swap(registros, i, j);
            ...
            if (esq < j) {
                quicksort(registros, esq, j, k);
            }
            if (i < dir && i < k) {
                quicksort(registros, i, dir, k);
            }
        }
    }
